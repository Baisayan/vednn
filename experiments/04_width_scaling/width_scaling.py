...
def main():
    ...
    for architecture in WIDTHS:
        label = "-".join(str(d) for d in architecture)
        print(f"\n--- Width: {label} ---")
    ...
    metrics = {
        ...
        "selected_width": best["architecture"],
        "results": results,
    }
    ...
    labels = ["-".join(str(d) for d in r["architecture"]) for r in results]
    ...
    plt.title("Width vs Best Validation Accuracy")
    for i, v in enumerate(val_accs):
        plt.text(i, v + 0.002, f"{v:.4f}", ha="center", fontsize=9)
    plt.grid(True, axis="y", alpha=0.3)
    acc_plot_path = plots_dir / "accuracy_vs_width.png"
    plt.tight_layout()
    ...
    plt.title("Width vs Training Time")
    for i, v in enumerate(times):
        plt.text(i, v + 0.5, f"{v:.1f}s", ha="center", fontsize=9)
    plt.grid(True, axis="y", alpha=0.3)
    time_plot_path = plots_dir / "time_vs_width.png"
    plt.tight_layout()
    ...
    for i, label in enumerate(labels):
        plt.text(params[i] + 5000, val_accs[i], label, fontsize=9)
    ...
    print("\n" + "=" * 30)
    print(f"Selected Width : {'-'.join(str(d) for d in best['architecture'])}")
