...
def build_model(optimizer, config):
    net = Network()
    seed = config["seed"]
    in_dim = 784
...
def main():
    ...
    for name, optimizer in OPTIMIZERS:
        print(f"\n--- Optimizer: {name} ---")
        model = build_model(optimizer, FIXED_CONFIG)
    ...
    metrics = {
        ...
        "selected_optimizer": best["optimizer"],
        "results": results,
    }
    ...
    plt.bar(x, val_accs, color=["royalblue", "orange", "green"])
    plt.xticks(x, names)
    plt.ylabel("Best Validation Accuracy")
    plt.title("Optimizer vs Best Validation Accuracy")
    for i, v in enumerate(val_accs):
        plt.text(i, v + 0.002, f"{v:.4f}", ha="center", fontsize=10)
    plt.grid(True, axis="y", alpha=0.3)
    acc_plot_path = plots_dir / "accuracy_vs_optimizer.png"
    plt.tight_layout()
    ...
    plt.figure(figsize=(8, 5))
    plt.bar(x, val_losses, color=["royalblue", "orange", "green"])
    plt.xticks(x, names)
    plt.ylabel("Best Validation Loss")
    plt.title("Optimizer vs Best Validation Loss")
    for i, v in enumerate(val_losses):
        plt.text(i, v + 0.002, f"{v:.4f}", ha="center", fontsize=10)
    plt.grid(True, axis="y", alpha=0.3)
    loss_plot_path = plots_dir / "loss_vs_optimizer.png"
    plt.tight_layout()
    ...
    print("\n" + "=" * 30)
    print(f"Selected Optimizer : {best['optimizer']}")
    print(f"Validation Accuracy : {best['best_validation_accuracy'] * 100:.2f}%")
    print(f"Validation Loss     : {best['best_validation_loss']:.4f}")
    print(f"Test Accuracy       : {best['test_accuracy'] * 100:.2f}%")
    print(f"Training Time       : {best['training_time_seconds']:.2f}s")
    print("=" * 30)
